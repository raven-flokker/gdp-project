<?php
		...
		if ($_POST['password'] == $_POST['confirm']) {
			...
			if (empty($user)) {
				...
				if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
					...
					$query = "INSERT INTO users SET login='$login', fio='$fio', pass='$password', email='$email', phone='$phone', status='0', banned='0', date=NOW()";
	...
	if (isset($_POST['login']) and isset($_POST['email']) and isset($_POST['fio'])/* and isset($_POST['trip-start'])*/) {
		$login = $_POST['login'];
		$email = $_POST['email'];
		$fio = $_POST['fio'];
...
<form id='forma' action='' method='POST'>
	...
	<div class="form-group">
		...
		<input class="form-control" id = "exampleInputName1" type="text" value="<?=$fio?>" placeholder="<?=LANG_FNAME?>" name='fio'>
	</div>
	<div class="form-group">
	...
	<div class="form-group">
    	...
    	<input type="tel" class="form-control" id="exampleInputPhone1" aria-describedby="phoneHelp" placeholder="" name='phone'>
